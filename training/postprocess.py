"""
Module for processing model outputs, peak detection, event and phase picking,
and saving results for seismogram analysis tasks.

Contains:
- _detect_peaks: Detect local peaks or valleys in 1D numpy arrays with various constraints.
- _detect_event: Detect event intervals in model output tensors based on probability thresholds.
- _pick_phase: Select phase picks (peaks) from model output tensors with constraints.
- process_outputs: Main function to process raw model outputs into structured results
  including detection and picking tasks.
- ResultSaver: Class to accumulate results from batches, process them, and save to CSV.

Functions operate on numpy arrays and PyTorch tensors and support batch processing.
"""
...
def _detect_event(
    ...
    """Detect event intervals in each sample of a batch output tensor using onset trigger.

    Args:
        outputs (torch.Tensor): Batch of model outputs (N x samples).
        prob_threshold (float): Minimum probability threshold to consider an event.
...
def _pick_phase(
    ...
    """
    Pick phase arrival indices from batch model outputs by detecting peaks.

    Args:
        outputs (torch.Tensor): Batch of model outputs (N x samples).
        prob_threshold (float): Minimum probability to qualify as a peak.
        min_peak_dist (int): Minimum allowed distance between picked peaks.
        topk (int): Only pick the phases whose probabilities are in the top k.
        padding_value (int): Value used to pad samples with fewer than topk picks.

    Returns:
        torch.Tensor: Tensor of shape (N, topk) containing peak indices per sample.
    """
    phases = []
    for output in outputs.detach().cpu().numpy():
...
def process_outputs(
    ...
    """
    Convert raw model outputs into processed results such as detection intervals,
    phase picks, and other task-specific outputs.

    Args:
        args (argparse.Namespace): Configuration arguments with thresholds etc.
        outputs (Union[Tuple[torch.Tensor], torch.Tensor]): Model output(s).
        label_names (List[str]): Names of output heads or labels.
        sampling_rate (int): Sampling rate of the input data.

    Returns:
        Dict[str, torch.Tensor]: Mapping from task names to processed output tensors.
    """
...
class ResultSaver:
    """
    Accumulate results from batches during inference/testing and save to CSV.

    Methods:
        - append: Add results and targets for a batch along with metadata.
        - save_as_csv: Save all accumulated results to a CSV file.

    Handles conversion of tensors to lists, one-hot decoding, removal of padding,
    and warns about unknown or missing keys.
    """
    ...
    def _convert_type(self, v: Union[list, torch.Tensor]) -> list:
        """Convert tensor or nested lists to list of strings suitable for saving."""
    ...
    def append(self, batch_meta_data: dict, targets: dict, results: dict) -> None:
        ...
        Args:
            batch_meta_data (dict): Metadata columns for each sample in batch.
            targets (dict): Ground truth targets for batch.
            results (dict): Model output results for batch.
        """
