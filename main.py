...
def get_args():
    """Parse and return the command-line arguments for model training/testing.

    This function parses command-line arguments to configure model training, testing,
    and dataset settings. It handles various parameters such as model name, checkpoint
    path, batch size, learning rate, and more. It also ensures compatibility with distributed
    training setups, logging, and other training/testing options.

    Returns:
        argparse.Namespace: The parsed arguments object containing the configuration
        parameters for model training or testing.

    Raises:
        ValueError: If the `p_position_ratio` is not in the range [0, 1].
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="MODEL_NAME",
        help="model name: 'phasenet/eqtransformer/magnet/ditingmotion/seist/team' (default: seist_l_dpk)",
    )
    parser.add_argument(
        "--checkpoint",
        default="",
    ...
    parser.add_argument(
        ...
        help="name of dataset ('diting', 'diting_light', 'pnw', 'pnw_light', 'sos' or stead) (default: 'stead')",
    )
    parser.add_argument(
        "--data-split",
        type=bool_,
    ...
    parser.add_argument(
        ...
        default=0.4,
        type=float,
        metavar="DROP_CH_RATE",
        help="drop channel rate (default:0.4)",
    )
    parser.add_argument(
        "--scale-amplitude-rate",
        default=0.4,
        type=float,
        metavar="SCALE_AMP_RATE",
    ...
    parser.add_argument(
        ...
        type=int,
        metavar="BATCH_SIZE",
        help="batch size (default: 512), this is the batch size of each worker (process)",
    )
    parser.add_argument(
        "--optim",
        default="Adam",
    ...
    parser.add_argument(
        ...
        default=True,
        type=bool_,
        metavar="USE_LR_SCHEDULER",
        help="whether use lr_scheduler (default: True)",
    )
    parser.add_argument(
        "--lr-scheduler-mode",
        default="exp_range",
        metavar="LR_SCHEDULER_MODE",
        type=str,
    ...
    parser.add_argument(
        ...
        default=0.5,
        type=float,
        metavar="DET_THRESHOLD",
        help="Probability threshold of DETection (default: 0.5)",
    )
    parser.add_argument(
        "--max-detect-event-num",
        default=1,
        type=int,
        metavar="MAX_DETECT_EV_NUM",
...
def main_worker(args, device):
    """The main worker for training and testing.

    This function handles the core logic of model training or testing. It initializes
    logging, sets up the device for computation, and calls the respective functions
    for training or testing based on the provided mode. It also handles distributed
    training setups and logs useful information about the training process.

    Args:
        args (argparse.Namespace): The parsed command-line arguments containing configuration settings.
        device (torch.device): The device (CPU or GPU) on which the model will run.

    Raises:
        ValueError: If `mode` is neither 'train', 'test', nor 'train_test'.

    """

    # Log directory and logger setup
    log_dir = (
        os.path.join(
            args.log_base, f"{get_time_str()}_{args.model_name}_{args.dataset_name}"
        )
...
if __name__ == "__main__":
    ...
    This function initializes argument parsing, distributed mode, sets the computation
    device, and then calls the main worker to start the training or testing process.
    """
    ...
    # Enable torch compilation if needed
